...
describe Config do
  ...
  context 'enumerable' do
    ...
    it 'should enumerate top level parameters' do
      keys = []
      config.each_key { |key| keys << key }
      expect(keys).to eq(%i[size section])
    end

    it 'should enumerate inner parameters' do
      keys = []
      config.section.each_key { |key| keys << key }
      expect(keys).to eq(%i[size servers])
    end

    it 'should have methods defined by Enumerable' do
      expect(config.map { |key, _value| key }).to eq(%i[size section])
    end
  end
    ...
    context 'using knockout_prefix' do
      ...
      context 'merging' do
        ...
        it 'should remove elements from settings' do
          expect(config.array1).to eq(%w[item4 item5 item6])
          expect(config.array2.inner).to eq(%w[item4 item5 item6])
